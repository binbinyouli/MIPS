/****************************************************************************
** Meta object code from reading C++ file 'previewwidget.h'
**
** Created: Sat Jun 25 10:32:34 2011
**      by: The Qt Meta Object Compiler version 62 (Qt 4.7.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../previewwidget.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'previewwidget.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 62
#error "This file was generated using the moc from 4.7.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_PreViewWidget[] = {

 // content:
       5,       // revision
       0,       // classname
       0,    0, // classinfo
       6,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: signature, parameters, type, tag, flags
      15,   14,   14,   14, 0x08,
      25,   14,   14,   14, 0x08,
      42,   37,   14,   14, 0x08,
      62,   14,   14,   14, 0x08,
      76,   14,   14,   14, 0x08,
      90,   14,   14,   14, 0x08,

       0        // eod
};

static const char qt_meta_stringdata_PreViewWidget[] = {
    "PreViewWidget\0\0zoomFit()\0zoomReset()\0"
    "rate\0zoomRateChange(int)\0zoomRateInc()\0"
    "zoomRateDec()\0rotateImage()\0"
};

const QMetaObject PreViewWidget::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_PreViewWidget,
      qt_meta_data_PreViewWidget, 0 }
};

#ifdef Q_NO_DATA_RELOCATION
const QMetaObject &PreViewWidget::getStaticMetaObject() { return staticMetaObject; }
#endif //Q_NO_DATA_RELOCATION

const QMetaObject *PreViewWidget::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->metaObject : &staticMetaObject;
}

void *PreViewWidget::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_PreViewWidget))
        return static_cast<void*>(const_cast< PreViewWidget*>(this));
    return QWidget::qt_metacast(_clname);
}

int PreViewWidget::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: zoomFit(); break;
        case 1: zoomReset(); break;
        case 2: zoomRateChange((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 3: zoomRateInc(); break;
        case 4: zoomRateDec(); break;
        case 5: rotateImage(); break;
        default: ;
        }
        _id -= 6;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
